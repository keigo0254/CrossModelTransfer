...
def full_finetune(rank: int, args: Args) -> ImageEncoder:
    """Finetune a pretrained model on multiple datasets."""
    if args.wandb:
        config_dict = vars(args)
        run = wandb.init(
            project="Task Arithmetic",
            group=f"full_finetune_{args.finetuning_type}",
    ...
    print("=" * 100)
    print(f"Full Finetuning {args.model_architecture} on {args.train_datasets} with {args.finetuning_type} finetuning")
    print("=" * 100)
    ...
    if args.save:
        ...
        filename = os.path.join(
            ...
            f"full_finetune_on_{args.pretrained}",
            f"bs_{args.batch_size}_seed_{args.seed}",
            f"{args.train_datasets}",
            f"{args.dataset_type}",
            f"augments_{args.augments}_lamb_{args.lamb}",
            f"num_images_{args.num_images}_num_augments_{args.num_augments}_beta_{args.beta}",
            f"full_finetuned_image_encoder_on_{args.train_datasets}_for_epochs_{args.epochs}.pt"
        )
        image_encoder = ImageEncoder(args, keep_lang=False)
        ...
        filename = os.path.join(
            ...
            f"augments_{args.augments}_lamb_{args.lamb}",
            f"num_images_{args.num_images}_num_augments_{args.num_augments}_beta_{args.beta}",
            f"full_finetuned_task_vector_on_{args.train_datasets}_for_epochs_{args.epochs}.pt"
        )
...
if __name__ == "__main__":
    ...
    args.result = os.path.join(
        ...
        f"augments_{args.augments}_lamb_{args.lamb}",
        f"num_images_{args.num_images}_num_augments_{args.num_augments}_beta_{args.beta}",
        f"full_finetuned_on_{args.train_datasets}_for_epochs_{args.epochs}.json"
    )
    args.fig = os.path.join(
        ...
        f"augments_{args.augments}_lamb_{args.lamb}",
        f"num_images_{args.num_images}_num_augments_{args.num_augments}_beta_{args.beta}",
        f"full_finetuned_on_{args.train_datasets}_for_epochs_{args.epochs}.jpg"
    )

    torch.multiprocessing.spawn(full_finetune, args=(args,), nprocs=args.world_size)

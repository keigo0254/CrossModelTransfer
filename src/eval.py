...
def eval_multihead_classifier(classifier: ImageClassifier | MultiHeadImageClassifier, args: Args):
    dataset = MixedDataset(args.eval_datasets, args.dataset_root, num_images=1000, preprocess=classifier.val_preprocess)
...
def eval_single_dataset(image_encoder: ImageEncoder, dataset_name: str, args: Args) -> float:
    classification_head = get_classification_head(args, dataset_name)
    model = ImageClassifier(image_encoder, classification_head)

    dataset = get_dataset(dataset_name, model.val_preprocess, location=args.dataset_root, batch_size=args.batch_size)
...
def evaluate(image_encoder: ImageEncoder, args: Args) -> dict:
